...
$conn_spe = mysqli_connect('localhost', 'root', '', 'cockbook') or die('Không thể kết nối đến CSDL');
mysqli_set_charset($conn_spe, 'utf8');

$page_profile_spe = isset($_GET['page_profile']) ? (int) $_GET['page_profile'] : 1;

if ($page_profile_spe < 1) {
    $page_profile_spe = 1;
}

$limit_profile_spe = 3;

$start_profile_spe = ($limit_profile_spe * $page_profile_spe) - $limit_profile_spe;

$sql_spe = "SELECT * from post_cockbook limit $start_profile_spe," . ($limit_profile_spe + 1);

$query_spe = mysqli_query($conn_spe, $sql_spe);

$result_spe = array();
while ($row_spe = mysqli_fetch_array($query_spe)) {
    array_push($result_spe, $row_spe);
}
$total_spe = count($result_spe);
if ($total_spe > $limit_profile_spe) {
    for ($i = 0; $i < $total_spe - 1; $i++) {
        ...
        <div class="item">
            ...
            <div class="item1 kfkrwjfikfjgi" style="margin-bottom:10px">
                <div class="item1-1 kusljcbsdmjvkc">
                    <img class="item1-1-1 ofhdedigjeorfkgdew" style="object-fit:cover" src="<?php echo $result_spe[$i]['avatar'] ?>" alt="">
                    <div class="item1-1-2">
                        <p class="item1-1-2-1"><?php echo $result_spe[$i]['name_post'] ?></p>
                        <p class="item1-1-2-2 jewrijkeqerwdgf lang" style="font-size: 12px" data-it-root="1m" data-it="1 phút ·">1m · &nbsp;<i data-visualcompletion="css-img" class="hu5pjgll m6k467ps" aria-label="Shared with Public group" role="img" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yk/r/woQDhi8VpyK.png&quot;); background-position: 0px -788px; background-size: 26px 928px; width: 12px; height: 12px; background-repeat: no-repeat; display: inline-block;"></i> &nbsp; · </p>
                    </div>
                </div>
                <!-- hide report  -->
                <style>
            ...
            <div class="item2" style="display:flex;padding: 12px 16px 0 16px">
                <div class="item2-1 amn<?php echo $result_spe[$i]['id'] ?>">
                    <p class="item2-1-1"><?php echo $result_spe[$i]['content'] ?></p>
                </div>
            </div>
            <style>
                .amn<?php echo $result_spe[$i]['id'] ?> {
                    height: <?php
                                    if ($result_spe[$i]['background'] !== '') {
                                        echo 300;
                                        echo 'px';
                                    } else {
                                        echo 0;
                                    ...
                                    ?>;
                    background-image: <?php echo $result_spe[$i]['background'] ?>;
                    background-position: center;
                    background-size: cover;
                    background-repeat: no-repeat;
                }
    ...
    <?php
        ...
        <style>
            ...
            .item1-1-2-2 i,
            .item1-2 i {
                filter: invert(100%) sepia(100%) saturate(0%) hue-rotate(157deg) brightness(103%) contrast(101%) !important;
            }
        </style>
        <div class="item">
            <style>
